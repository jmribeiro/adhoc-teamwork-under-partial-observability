import pygame
import os, copy, math
from backend.environments.overcooked_ai_py.utils import generate_temporary_file_path, classproperty, cumulative_rewards_from_rew_list
from backend.environments.overcooked_ai_py.static import GRAPHICS_DIR, FONTS_DIR
from backend.environments.overcooked_ai_py.mdp.layout_generator import EMPTY, COUNTER, ONION_DISPENSER, TOMATO_DISPENSER, POT, DISH_DISPENSER, SERVING_LOC
from backend.environments.overcooked_ai_py.visualization.visualization_utils import show_image_in_ipython, show_ipython_images_slider
from backend.environments.overcooked_ai_py.visualization.pygame_utils import MultiFramePygameImage, run_static_resizeable_window, vstack_surfaces, scale_surface_by_factor, blit_on_new_surface_of_size
from backend.environments.overcooked_ai_py.mdp.actions import Direction, Action
...
class StateVisualizer:
    ...
    def render_clean_state(self, players, objects, grid, img_path=None, show=True):
        ...
        if result_surface_size != rendered_surface.get_size():
            ...
            result_surface = rendered_surface
        if img_path is not None:
            pygame.image.save(result_surface, img_path)
        if show:
            run_static_resizeable_window(result_surface, fps=30)

    def render_clean_grid(self, grid, filename=None, show=True):
        surface = self.render_grid(grid)
        if filename is not None:
            pygame.image.save(surface, filename)
        if show:
            run_static_resizeable_window(surface, fps=30)
