...
class OvercookedPOMDP(PartiallyObservableMarkovDecisionProcess):
    ...
    def render(self, mode='human'):
        ...
        p1, p2, hand_1, hand_2, top_balcony, bottom_balcony, soup_pan = self.state
        GRID = [
            ["X", "X", "X", "P", "X"],
            ["O", " ", "X", " ", "X"],
            ["D", " ", "X", " ", "X"],
...
SOUP_DELIVERED = (42, 42, 42, 42, 42, 42, 42)
ABSORBENT = (41, 41, 41, 41, 41, 41, 41)
...
def generate_mdp(teammate):
    ...
    R = generate_rewards(states, tuple(range(len(ACTION_MEANINGS))))
...
def generate_mmdp(teammate):
    ...
    gamma = 0.95
    R = generate_rewards(states, JOINT_ACTION_SPACE)
...
def generate_state_space():
    ...
    states = [
        np.array((p1, p2, hand_1, hand_2, top_balcony, bottom_balcony, soup_pan))
    ...
    states = states + [np.array(SOUP_DELIVERED), np.array(ABSORBENT)]
    return states

def generate_mmdp_transition_probabilities_cube(states):

    def next_position(current_position, action_meaning):
        if current_position == TOP and action_meaning == "down": return BOTTOM
        elif current_position == BOTTOM and action_meaning == "up": return TOP
        else: return current_position

    def regular_transition(state, action_meaning_1, action_meaning_2):

        p1, p2, hand_1, hand_2, top_balcony, bottom_balcony, soup_pan = state
        ...
        if next_soup_delivered:
            next_state = np.array(SOUP_DELIVERED)
        else:
            next_state = np.array((next_p1, next_p2, next_hand_1, next_hand_2, next_top_balcony, next_bottom_balcony, next_soup_pan))

        return next_state

    def transition(state, action_meaning_1, action_meaning_2):

        soup_delivered = tuple(state) == SOUP_DELIVERED
        game_ended = tuple(state) == ABSORBENT

        # Regular transition
        if not soup_delivered and not game_ended:
            next_state = regular_transition(state, action_meaning_1, action_meaning_2)
        else:
            next_state = np.array(ABSORBENT)
    ...
    for a0, action_meaning_0 in enumerate(ACTION_MEANINGS):
        for a1, action_meaning_1 in enumerate(ACTION_MEANINGS):
            ...
            for x, state in enumerate(states):
                ja = JOINT_ACTION_SPACE.index(joint_action)
                y = transition(state, action_meaning_0, action_meaning_1)
                P[ja, x, y] = 1.0
...
def generate_rewards(states, actions):
    ...
    for x, state in enumerate(states):
        if tuple(state) == SOUP_DELIVERED:
            R[x, :] = 100
        elif tuple(state) == ABSORBENT:
            R[x, :] = 0.0
        else:
            R[x, :] = -1.0
    return R
...
def generate_miu(states):
    ...
    def valid_initial_state(state):
        p1, p2, hand_1, hand_2, top_balcony, bottom_balcony, soup_pan = state
        if tuple(state) == ABSORBENT or tuple(state) == SOUP_DELIVERED:
            return False
        else:
            return\
                hand_1 == NOTHING and \
