...
class LinePlotHourglass:
    ...
    def show(self, error_fill=True, error_fill_transparency=0.25, ticks=None):
        if len(self._stats) > 0:
            if self._current_figure is not None: plt.close(self._current_figure)
            self._make_fig(error_fill, error_fill_transparency, ticks=ticks)
            self._current_figure.show()
            plt.close(self._current_figure)
            del self._current_figure
            self._current_figure = None

    def savefig(self, filename=None, error_fill=True, error_fill_transparency=0.25, ticks=None):
        if len(self._stats) > 0:
    ...
    def _make_fig(self, error_fill=True, error_fill_transparency=0.25, show_legend=True, ticks=None):
        ...
        for agent_name, stats in self._stats.items():
            ...
            ax.errorbar(x_ticks, means, yerr=errors, ls='dotted', marker=marker, color=color)
