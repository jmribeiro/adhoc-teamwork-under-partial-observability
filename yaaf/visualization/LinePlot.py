...
class LinePlot:
    ...
    def show(self, error_fill=True, error_fill_transparency=0.25, ticks=None, show_legend=True):
        if len(self._stats) > 0:
            if self._current_figure is not None: plt.close(self._current_figure)
            self._make_fig(error_fill, error_fill_transparency, ticks=ticks, show_legend=show_legend)
            self._current_figure.show()
            plt.close(self._current_figure)
            del self._current_figure
            self._current_figure = None

    def savefig(self, filename=None, error_fill=True, error_fill_transparency=0.25, ticks=None, show_legend=True):
        if len(self._stats) > 0:
    ...
    def _make_fig(self, error_fill=True, error_fill_transparency=0.25, show_legend=True, ticks=None):

        x_ticks = (np.arange(self._x_ticks) + 1) * self._x_tick_step if ticks is None else ticks
        ...
        for agent_name, stats in self._stats.items():
            ...
            argmax_mean = means.argmax()
            max_mean = means[argmax_mean]
            max_error = stats.errors[argmax_mean]
            ceil = max_mean + max_error
            ceiling = max(ceil, ceiling)

            color = self._colors[agent_name]
            marker = self._markers[agent_name]

            if self._legend[agent_name]:
                ax.plot(x_ticks, means, lw=2, label=f"{agent_name}", color=color, marker=marker)
            else:
        ...
        # LOG SCALE RANDOM AGENT
        #if "Random" in self._stats:
        #ax.set_yscale("log")
